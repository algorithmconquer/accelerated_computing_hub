 ...
 * 1. 朴素归约（原子操作）
 * 2. 共享内存归约（树形归约）
 * 3. 循环展开归约（Warp级优化） - 修复版
 * 4. Warp Shuffle归约（高级）
 * 
 * 修复内容：
 * - 修复方法3（循环展开归约）的Warp同步问题
 */
...
__global__ void reduceUnrolled(float* input, float* partialSums, int n) {
    ...
    // 
    // 【关键修复】：
    // 1. 添加__syncwarp()确保Warp内同步
    // 2. 使用volatile避免编译器重排序
    // 3. 确保每次加法完成后才进行下一步
    if (tid < 32) {
        // 使用volatile指针，避免编译器优化导致的数据不一致
        volatile float* vsdata = sdata;
        
        // 手动展开最后5轮（32 -> 16 -> 8 -> 4 -> 2 -> 1）
        // 每一步都需要同步，确保前面的加法完成
        vsdata[tid] += vsdata[tid + 32];  // 32 -> 16
        __syncwarp();                      // ✓ 添加Warp同步
        
        vsdata[tid] += vsdata[tid + 16];  // 16 -> 8
        __syncwarp();                      // ✓ 添加Warp同步
        
        vsdata[tid] += vsdata[tid + 8];   // 8 -> 4
        __syncwarp();                      // ✓ 添加Warp同步
        
        vsdata[tid] += vsdata[tid + 4];   // 4 -> 2
        __syncwarp();                      // ✓ 添加Warp同步
        
        vsdata[tid] += vsdata[tid + 2];   // 2 -> 1
        __syncwarp();                      // ✓ 添加Warp同步
        
        vsdata[tid] += vsdata[tid + 1];   // 最后一个元素
        __syncwarp();                      // ✓ 添加Warp同步
    }
...
int main() {
    ...
    printf("\n修复说明:\n");
    printf("1. ✓ 在循环展开归约中添加__syncwarp()确保Warp内同步\n");
    printf("2. ✓ 使用volatile避免编译器重排序\n");
    printf("3. ✓ 确保每次加法完成后才进行下一步\n");
 ...
 * 
 * 编译:
 *   nvcc -O3 -arch=sm_80 reduction_comparison_fix1.cu -o reduction_fix1
 * 
 * 运行:
 *   ./reduction_fix1
 * 
 * 性能分析:
 *   nvprof ./reduction_fix1
 * 
 * ========== 预期输出 ==========
 * 
 * 方法1: ~48 ms, 带宽 ~1.4 GB/s
 * 方法2: ~0.46 ms, 带宽 ~145 GB/s, 加速 ~104x
 * 方法3: ~0.26 ms, 带宽 ~260 GB/s, 加速 ~187x (修复后结果正确)
 * 方法4: ~0.25 ms, 带宽 ~270 GB/s, 加速 ~194x
 */
